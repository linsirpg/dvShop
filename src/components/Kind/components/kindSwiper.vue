    ...
    return {
      CatetorySwiper: {
        ...
        on: {
          touchStart () {
            ...
            if (this.app && this.app.Data) {
              if (this.app.Data[this.activeIndex + 1]) {
                this.slides.eq(this.activeIndex + 1).css('transform', 'translate3d(0,' + (ScTop - this.app.$store.state[this.app.$route.name + 'ScrollTop' + this.app.Data[this.activeIndex + 1].Id]) + 'px, 0)')
              }
              if (this.app.Data[this.activeIndex - 1]) {
                this.slides.eq(this.activeIndex - 1).css('transform', 'translate3d(0,' + (ScTop - this.app.$store.state[this.app.$route.name + 'ScrollTop' + this.app.Data[this.activeIndex - 1].Id]) + 'px, 0)')
              }
          ...
          transitionEnd () {
            var routerFrom = this.app.Data[this.activeIndex].Id
            ...
            if (this.app && this.app.Data) {
              ...
              if (this.app.LimitHeight < H) {
                this.app.LimitHeight = H
              }
              document.getElementsByClassName('Kind')[0].style.height = this.app.LimitHeight + 'px'
              document.documentElement.scrollTop = document.body.scrollTop = this.app.$store.state[this.app.$route.name + 'ScrollTop' + this.app.Data[this.activeIndex].Id]
          ...
          This.Data.map(function (data, index) {
            if (data.Id === This.$route.query.id) {
              ...
              if (This.ProductData[index].length) {
                ...
                var H = document.documentElement.clientHeight || document.body.clientHeight
                if (This.LimitHeight < H) {
                  This.LimitHeight = H
                }
                document.getElementsByClassName('Kind')[0].style.height = This.LimitHeight + 'px'
                document.getElementsByClassName('Kind')[0].style.overflow = 'hidden'
  ...
  methods: {
    CatetoryScroll () {
      ...
      if (document.documentElement.scrollHeight - document.documentElement.scrollTop - document.documentElement.clientHeight < 300 || document.body.scrollHeight - document.body.scrollTop - document.documentElement.clientHeight < 300) {
        this.$store.dispatch('getMoreKindData', {id: this.$route.query.id, CateName: this.$route.params.Id, CatetoryId: this.$route.params.id, Name: this.$route.name, limit: this.$route.meta.limit})
      }
      if (This.CatetorySwiperArr && This.Data && This.Data.length) {
        This.Data.map(function (data, index) {
          if (data.Id === This.$route.query.id) {
